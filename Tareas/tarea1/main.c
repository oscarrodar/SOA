...
*Programa que recibe como parámetro otro programa y muestra en pantalla la información de los system calls llamados
...
void printTable(long* list, int* count); 
const char* getSystemCallName(long systemCallNumber);
void showMessage(short messageType);
void addToTable(long value, long* list, int* count);
...
int main(int argc, char* argv[]) {   
  ...
  short rastrear = 0; // Solo muestra resultado final
  int i = 0;
  if (argv[1][0] == '-' && argv[1][1] == 'v'){
	rastrear = 1; //Muestra info de cada System Call que se encuentra
        i++;
  }else if (argv[1][0] == '-' && argv[1][1] == 'V'){
	rastrear = 2; //Muestra info de cada System Call y espera una tecla para continuar
        i++;
  }
  ...
  if(child == 0) {
	...
	printTable(sysCalls, sysCallsContador);
...
void showMessage(short messageType){
	switch(messageType){
		case 1: fprintf(stderr, "No se a podido ejecutar el rastreador. Se espera el siguiente formato:\nrastreador [rastreador param] programa [programa params]\n");
...
void printTable(long* list, int* count){
	...
	fprintf(stderr, "*******************************************************************************\n");
	fprintf(stderr, "Sys call #\t\t# de llamadas\t\tNombre\n");
	int i = 0;
	while (list[i] != 0){
		fprintf(stderr, "%ld\t\t\t%d\t\t\t%s\n", list[i],count[i],getSystemCallName(list[i]));
		i++;
	}
	fprintf(stderr, "*******************************************************************************\n");
}
...
/*
*Esta funcion de mapeo entre el número del System Call y su nombre, fue tomado de https://gist.github.com/willb/14488
*/
